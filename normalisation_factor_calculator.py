...
class Normalisation_Factor_Calculator:
	...
	def Check_NF(self, T_c, NF, I0_m, I0_sensitivity):
		...
		if I0_scale_diff_percent <= self.I0_sensitivity:
			...
			SF = {"results" : "No action required for I0 as Mw (kD)", "Scaled NF" : self.NF, "Absolute NF" : self.calculate_absolute_normalisation()}
			return  SF
			...
			SF = {"results" :"No action required for absolute scale (cm-1)", "Scaled NF": self.calculate_scaled_normalisation(self.calculate_absolute_normalisation()), "Absolute NF" : self.NF}
			return  SF
		else:
			...
			print "New normalization factor for absolute scale (cm-1) : %s" % (self.calculate_absolute_normalisation())
			print "New normalization factor for for I0 as Mw (kD) : %s" % (self.calculate_scaled_normalisation(self.calculate_absolute_normalisation()))
			SF = {"results" : "NF does not match either Absolute or Scaled: Please update", "Scaled NF" : self.calculate_scaled_normalisation(self.calculate_absolute_normalisation()), "Absolute NF" : self.calculate_absolute_normalisation()}
			return SF
